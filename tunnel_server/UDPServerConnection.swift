...
class UDPServerConnection: Connection {
    ...
    deinit {
		if let responseSource = responseSource {
		...
		switch addressFamily {
			case AF_INET:
                ...
                sent = withUnsafePointer(to: &serverAddress.sin) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                        sendto(UDPSocket, data.withUnsafeBytes { $0.baseAddress }, data.count, 0, $0, socklen_t(serverAddress.sin.sin_len))
                    }
				}
			...
			case AF_INET6:
                ...
                sent = withUnsafePointer(to: &serverAddress.sin6) {
                    $0.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                        sendto(UDPSocket, data.withUnsafeBytes { $0.baseAddress }, data.count, 0, $0, socklen_t(serverAddress.sin6.sin6_len))
                    }
				}
